...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fi="http://apache.org/cocoon/forms/1.0#instance" exclude-result-prefixes="fi">
  <!--+
	...
	<xsl:template match="head" mode="forms-calendar">
		<script src="{$resources-uri}/mattkruse-lib/ameba-CalendarPopup.js" type="text/javascript"/>
      ...
      forms_calendar.setCssPrefix('forms_');				
		...
		<link rel="stylesheet" type="text/css" href="{$resources-uri}/ameba-forms-calendar.css"/>
	</xsl:template>
  ...
  <!--+
      ...
      +-->
	<xsl:template match="fi:field[fi:styling/@type='date'] | fi:field[not(fi:styling/@type)][fi:datatype[@type='date']][not(fi:selection-list)]">
    ...
    <!-- calendar popup -->
		<a href="#" name="{$id}" id="{$id}" onClick="forms_calendar.select(forms_getForm(this)['{@id}'],'{$id}','{$format}'); return false;">
