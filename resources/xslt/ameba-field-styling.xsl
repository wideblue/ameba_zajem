...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fi="http://apache.org/cocoon/forms/1.0#instance" exclude-result-prefixes="fi">
  ...
  <!-- Location of the resources directory, where JS libs and icons are stored -->
	...
	<xsl:template match="head" mode="forms-field">
		<script src="{$resources-uri}/ameba-forms-lib.js" type="text/javascript"/>
		<link rel="stylesheet" type="text/css" href="{$resources-uri}/ameba-forms.css"/>
	</xsl:template>
  ...
  <!--+
	...
	<xsl:template match="fi:field">
		<input class="forms-field" name="{@id}" id="{@id}" value="{fi:value}" title="{fi:hint}" type="text">
  ...
  <!--+
    ...
    <!--+ 
        ...
        +-->
		<xsl:if test="self::fi:field[fi:selection-list][fi:styling/@list-type = 'listbox'] or self::fi:multivaluefield[not(fi:styling/@list-type = 'checkbox')]">
	...
	<xsl:template match="fi:styling/@list-type | fi:styling/@list-orientation | fi:styling/@listbox-size | fi:styling/@format | fi:styling/@layout" mode="styling">
    <!--+
    ...
    <!--+ 
        ...
        +-->
		<xsl:variable name="validHTMLTypes" select="'text hidden textarea checkbox radio password image reset submit'"/>
		<xsl:if test="normalize-space(.) and contains(concat(' ', $validHTMLTypes, ' '), concat(' ', ., ' '))">
			<xsl:copy-of select="."/>
		</xsl:if>
	</xsl:template>
  ...
  <!--+
    ...
    <!-- dropdown or listbox -->
		<select title="{fi:hint}" id="{@id}" name="{@id}" class="forms-select">
			<xsl:apply-templates select="." mode="styling"/>
			<xsl:for-each select="fi:selection-list/fi:item">
				<option value="{@value}" class="forms-option">
					<xsl:if test="@value = $value">
  ...
  <!--+
	...
	<xsl:template match="fi:booleanfield[fi:styling/@type='output']">
		<xsl:choose>
			<xsl:when test="fi:value = 'true'">yes</xsl:when>
  ...
  <!--+
	...
	<xsl:template match="fi:action[fi:styling/@type = 'link']"  priority="1">
	  <a id="{@id}" title="{fi:hint}" href="javascript://" onClick="forms_submitForm(this, '{@id}'); return false;"> 	 
      <xsl:apply-templates select="." mode="styling"/> 	 
      <xsl:copy-of select="fi:label/node()"/>
		</a>
  </xsl:template>
	 ...
	 <a id="{@id}" title="{fi:hint}" href="javascript://" onClick="forms_submitForm(this, '{@id}'); return true;">
	<xsl:template match="fi:action[fi:styling/@type = 'link']" >
      ...
      <xsl:copy-of select="fi:label/node()"/>
	   ...
	   <xsl:template match="fi:action[fi:styling/@type='link']">
		<a id="{@id}" name="{@id}" href="#" onclick="forms_submitForm(this)">
			<xsl:value-of select="fi:label/node()"/>
			<xsl:apply-templates select="." mode="styling"/>
		</a>
	</xsl:template>
