...
<xsl:stylesheet version="1.0"
  ...
  <!-- Location of the resources directory, where JS libs and icons are stored -->
	...
	<xsl:template match="body" mode="forms-field">
		<xsl:copy-of select="@*"/>
		<xsl:attribute name="onload">forms_onload(); <xsl:value-of select="@onload"/>
		</xsl:attribute>
	</xsl:template>
  ...
  <!--+
	...
	<xsl:template match="fi:styling/@list-type | fi:styling/@list-orientation | fi:styling/@listbox-size | fi:styling/@format | fi:styling/@layout"
			mode="styling">
    <!--+
              ...
              <td>
                <input type="radio" id="{generate-id()}" name="{$id}" value="{@value}">
                  ...
                  <xsl:apply-templates select="." mode="styling"/>
							...
							<td>
                <label for="{generate-id()}"><xsl:copy-of select="fi:label/node()"/>
								</label>
              </td>
  ...
  <!--+
	...
	<xsl:template match="fi:form-template|fi:form-generated">
		<form name="form">
			<xsl:copy-of select="@*"/>
			<xsl:attribute name="onsubmit">forms_onsubmit();<xsl:value-of select="@onsubmit"/> 
			</xsl:attribute>
